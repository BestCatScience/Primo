...
extension DocumentStrokeRasterizer {
    ...
    private static func interpolate(_ from: CGFloat, _ to: CGFloat, amount: CGFloat) -> CGFloat {
        from + ((to - from) * amount)
    }

    private static func rasterizedLuminance(red: CGFloat, green: CGFloat, blue: CGFloat) -> CGFloat {
    ...
    private static func sampleRasterizedSmearingColor(
        ...
        let bleedRadius = Self.clampUnit(CGFloat(brush.smudgeRadius))
        let wetness = Self.clampUnit(CGFloat(brush.wetness))
    ...
    private static func sampleRasterizedDullingColor(
        ...
        let bleedRadius = Self.clampUnit(CGFloat(brush.smudgeRadius))
        let wetness = Self.clampUnit(CGFloat(brush.wetness))
        let mixStrength = Self.clampUnit(CGFloat(brush.colorMixStrength))
    ...
    private static func sampleRasterizedBlurColor(
        ...
        let blurAmount = Self.clampUnit(CGFloat(brush.smudgeBleed))
        let blurRadius = Self.clampUnit(CGFloat(brush.smudgeRadius))
        let mixStrength = Self.clampUnit(CGFloat(brush.colorMixStrength))
    ...
    static func blendRasterizedPixel(
        ...
        let pigmentDensity = Self.clampUnit(CGFloat(brush.flow))
        ...
        let clampedPressure = Self.clampUnit(max(0.08, min(pressure, 1.0)))
        let lowPressureMix = Self.clampUnit((1.0 - clampedPressure) / 0.92)
        let activeBlurAmount = blurEnabled ? blurAmount : 0
        let activeBlurRadius = blurEnabled ? blurRadius : 0
        let effectiveColorStretch = Self.clampUnit(colorStretch * (0.74 + (lowPressureMix * 0.82)))
        let effectiveUndercoatMix = Self.clampUnit(undercoatMix * (0.78 + (lowPressureMix * 0.72)))
        let effectiveBlurAmount = Self.clampUnit(activeBlurAmount * (0.72 + (lowPressureMix * 0.88)))
        let effectiveBlurRadius = Self.clampUnit(activeBlurRadius * (0.84 + (lowPressureMix * 0.44)))
        let spacingFactor = min(max((CGFloat(brush.stampSpacing) - 0.02) / 0.38, 0), 1)
        let smudgeOpacityGate = Self.clampUnit((sourceAlpha - 0.04) / 0.86)
        let mixActivation = Self.clampUnit(smudgeOpacityGate + (lowPressureMix * 0.42))
        let edgeColorGuard = Self.clampUnit((sourceAlpha - 0.10) / 0.36)
        let contourFade = Self.clampUnit((sourceAlpha - 0.20) / 0.44)
        let corePresence = Self.clampUnit((sourceAlpha - 0.46) / 0.34)
        let edgeMixGuardBase = Self.clampUnit(edgeColorGuard + (lowPressureMix * 0.20))
        let edgeMixGuard = edgeMixGuardBase * (0.18 + (contourFade * 0.82))
        let edgeAlphaGuard = Self.clampUnit((max(destinationAlpha, sourceAlpha) - 0.02) / 0.16)
        let colorRate = Self.clampUnit(CGFloat(brush.paintLoad))
        let effectiveColorRate = Self.clampUnit(colorRate * (0.24 + (clampedPressure * 0.76)))
        let effectivePigmentDensity = Self.clampUnit(pigmentDensity * (0.16 + (clampedPressure * 0.84)))
        let smudgeEnabled =
            mode != .off &&
            (
        ...
        if smudgeEnabled {
            ...
            if !transparentSurface {
                let smearStrength = Self.clampUnit(
                    mixActivation *
                    (0.14 + (effectiveColorStretch * 0.98) + ((1.0 - spacingFactor) * 0.10))
                )
                let dullStrength = Self.clampUnit(
                    mixActivation *
                    (0.04 + (effectiveUndercoatMix * 0.94) + (effectiveBlurAmount * 0.10))
                )
                var mixedPremulRed = Self.interpolate(destinationPremulRed, smearSample.red, amount: smearStrength)
                var mixedPremulGreen = Self.interpolate(destinationPremulGreen, smearSample.green, amount: smearStrength)
                var mixedPremulBlue = Self.interpolate(destinationPremulBlue, smearSample.blue, amount: smearStrength)
                var mixedAlpha = Self.interpolate(destinationAlpha, smearSample.alpha, amount: smearStrength)
                mixedPremulRed = Self.interpolate(mixedPremulRed, dullSample.red, amount: dullStrength)
                mixedPremulGreen = Self.interpolate(mixedPremulGreen, dullSample.green, amount: dullStrength)
                mixedPremulBlue = Self.interpolate(mixedPremulBlue, dullSample.blue, amount: dullStrength)
                mixedAlpha = Self.interpolate(mixedAlpha, dullSample.alpha, amount: dullStrength)

                if blurEnabled && (effectiveBlurAmount > 0.001 || effectiveBlurRadius > 0.001) {
                    let blurStrength = Self.clampUnit(
                        mixActivation *
                        (
                            0.02 +
                    ...
                    mixedPremulRed = Self.interpolate(mixedPremulRed, blurRed, amount: blurStrength)
                    mixedPremulGreen = Self.interpolate(mixedPremulGreen, blurGreen, amount: blurStrength)
                    mixedPremulBlue = Self.interpolate(mixedPremulBlue, blurBlue, amount: blurStrength)
                    mixedAlpha = Self.interpolate(mixedAlpha, blurAlpha, amount: blurStrength)
                }
                ...
                let mixedLuminance = Self.rasterizedLuminance(red: mixedRed, green: mixedGreen, blue: mixedBlue)
                let pigmentLuminance = Self.rasterizedLuminance(red: sourceRed, green: sourceGreen, blue: sourceBlue)
                let luminanceSimilarity = Self.clampUnit(1.0 - abs(mixedLuminance - pigmentLuminance) * 0.38)
                let undercoatBlendInput =
                    edgeMixGuard *
                    (0.18 + (effectiveUndercoatMix * 0.82)) *
                    luminanceSimilarity *
                    (1.0 - (corePresence * 0.18))
                let undercoatBlend = Self.clampUnit(undercoatBlendInput)
                let baseBlend = (mode == .smear ? edgeMixGuard : undercoatBlend) * (1.0 - (corePresence * 0.12))
                let centerPigmentRetention = Self.clampUnit(effectivePigmentDensity * (1.0 + (corePresence * 0.30)))
                let brushBaseRed = Self.interpolate(mixedRed, sourceRed, amount: centerPigmentRetention)
                let brushBaseGreen = Self.interpolate(mixedGreen, sourceGreen, amount: centerPigmentRetention)
                let brushBaseBlue = Self.interpolate(mixedBlue, sourceBlue, amount: centerPigmentRetention)
                let smudgedRed = Self.interpolate(brushBaseRed, mixedRed, amount: baseBlend)
                let smudgedGreen = Self.interpolate(brushBaseGreen, mixedGreen, amount: baseBlend)
                let smudgedBlue = Self.interpolate(brushBaseBlue, mixedBlue, amount: baseBlend)
                let pigmentInfluence = Self.clampUnit(
                    ((mode == .smear ? min(effectiveColorRate, 0.28) : effectiveColorRate) * 0.96) +
                    ((1.0 - edgeAlphaGuard) * 0.04)
                ) * centerPigmentRetention
                resolvedSourceRed = Self.interpolate(smudgedRed, sourceRed, amount: pigmentInfluence)
                resolvedSourceGreen = Self.interpolate(smudgedGreen, sourceGreen, amount: pigmentInfluence)
                resolvedSourceBlue = Self.interpolate(smudgedBlue, sourceBlue, amount: pigmentInfluence)

                let smudgedCoverage = Self.clampUnit(
                    (smearSample.alpha * 0.44) +
                    (dullSample.alpha * 0.26) +
                    (blurSample.alpha * 0.30)
                )
                let coverageInfluenceInput =
                    (1.0 - effectiveColorRate) *
                    (0.24 + (effectiveColorStretch * 0.40) + (effectiveUndercoatMix * 0.22) + (lowPressureMix * 0.14)) *
                    (0.92 + ((1.0 - contourFade) * 0.52)) *
                    (1.0 - (corePresence * 0.42))
                let coverageInfluence = Self.clampUnit(coverageInfluenceInput)
                let coverageFloorLow = 0.02 + (effectiveColorRate * 0.06)
                let coverageFloorHigh = 0.24 + (effectiveColorRate * 0.26) + (corePresence * (0.10 + (effectiveColorRate * 0.08)))
                let coverageFloor = min(0.92, Self.interpolate(coverageFloorLow, coverageFloorHigh, amount: contourFade))
                let coverage = max(coverageFloor, Self.interpolate(1.0, smudgedCoverage, amount: coverageInfluence))
