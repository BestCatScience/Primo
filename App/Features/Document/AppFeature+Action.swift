...
extension AppFeature.Action {
    ...
    static func workspacePersistenceRequested(_ request: AppFeature.WorkspacePersistenceRequest) -> Self { .workspace(.persistenceRequested(request)) }
    static func workspacePersistenceSucceeded(_ result: AppFeature.WorkspacePersistenceResult) -> Self { .workspace(.persistenceSucceeded(result)) }
    static func workspacePersistenceFailed(_ failure: AppFeature.WorkspacePersistenceFailure) -> Self { .workspace(.persistenceFailed(failure)) }
    static func workspaceCatalogRequested(_ request: AppFeature.WorkspaceCatalogRequest) -> Self { .workspace(.catalogRequested(request)) }
    static func workspaceCatalogSucceeded(_ result: AppFeature.WorkspaceCatalogResult) -> Self { .workspace(.catalogSucceeded(result)) }
    static func workspaceCatalogFailed(_ failure: AppFeature.WorkspaceCatalogFailure) -> Self { .workspace(.catalogFailed(failure)) }
    ...
    static func newCanvasFromImagePreparationCompleted(_ plan: AppFeature.ImportedCanvasPlan) -> Self { .importExport(.newCanvasFromImagePreparationCompleted(plan)) }
