...
extension PrimoRootFeature {
    struct CanvasRenderStateCoordinator {
        ...
        func applyCanvasPresentation(
            ...
            to state: inout PrimoRootFeature.State
        ) {
    ...
    struct CanvasInteractionStateCoordinator {
        func syncPresentation(
            _ presentation: PaintDocumentPresentation,
            state: inout PrimoRootFeature.State
        ) {
            syncCanvasInteractionState(state: &state)
            syncActiveTextLayer(from: presentation, state: &state)
        }

        func syncCanvasInteractionState(state: inout PrimoRootFeature.State) {
            state.canvas.updateInteractionStyle(
                previewStyle: PrimoRootFeature.canvasToolStateCoordinator.previewStrokeStyle(for: state),
                paperStyle: PrimoRootFeature.canvasToolStateCoordinator.resolvedPaperStyle(for: state)
            )
        ...
        func syncTextEditorWithActiveLayer(state: inout PrimoRootFeature.State) {
    ...
    struct FreshDocumentStateCoordinator {
        func prepare(
            canvasSize: CGSize,
            to state: inout PrimoRootFeature.State
        ) {
            ...
            PrimoRootFeature.toolPanelStateCoordinator.resetPanels(in: &state)
    ...
    struct CanvasPresentationStateCoordinator {
        ...
        let freshDocumentCoordinator = FreshDocumentStateCoordinator()
