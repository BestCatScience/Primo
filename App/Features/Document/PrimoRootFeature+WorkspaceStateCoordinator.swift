...
extension PrimoRootFeature.WorkspaceState {
    ...
    mutating func closeTab(
        id tabID: OpenDocumentTab.ID
    ) -> PrimoRootFeature.WorkspaceTabClosureResult? {
        let wasActive = isActiveTab(tabID)
        guard let closingTab = removeTab(id: tabID) else { return nil }

        guard wasActive else {
            return PrimoRootFeature.WorkspaceTabClosureResult(
    ...
    ) -> PrimoRootFeature.WorkspacePaneActivationDisposition {
        focus(on: pane)
